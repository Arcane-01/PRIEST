...
class batch_crowd_nav():
    ...
    def __init__(self, a_obs, b_obs, v_max, v_min, a_max, num_obs, t_fin, num, num_batch, maxiter, maxiter_cem, weight_smoothness, weight_track, way_point_shape, v_des):
        ...
        self.A_obs = jnp.tile(self.P_jax, (self.num_obs_proj, 1))
        ...
        self.scale_factor = np.random.normal( 0, 0.8, size=self.num_batch * self.initial_up_sampling)
    ...
    ##############################################
    @partial(jit, static_argnums=(0,),backend="gpu")
    def compute_obs_traj_prediction(self, x_obs_init, y_obs_init, vx_obs, vy_obs, x_init, y_init):
        ...
        x_obs_trajectory = x_obs_trajectory[idx_dis[0:(self.num_obs)],:]
        y_obs_trajectory = y_obs_trajectory[idx_dis[0:(self.num_obs)],:]
        ...
        return x_obs_trajectory, y_obs_trajectory, x_obs_trajectory_proj, y_obs_trajectory_proj
    ...
    def compute_warm_traj(self, initial_state, v_des, x_waypoint, y_waypoint, arc_vec, x_diff, y_diff):
        ...
        # x_guess_per = (x_interp + 0.0*self.eps_k)[0:self.num_sample_warm, :]
        # y_guess_per = (y_interp + self.eps_k)[0:self.num_sample_warm, :]
    ...
    @partial(jit, static_argnums=(0,),backend="gpu")
    def compute_traj_guess(self, initial_state, x_obs_trajectory, y_obs_trajectory, v_des, x_waypoint, y_waypoint, arc_vec, x_guess_per , y_guess_per, x_diff, y_diff):
        ...
        cost_obs_penalty = jnp.linalg.norm(jnp.maximum(jnp.zeros(( self.initial_up_sampling*self.num_batch, self.num*self.num_obs  )), dist_obs), axis = 1)# jnp.linalg.norm(1.6*jnp.minimum(jnp.zeros(( self.initial_up_sampling*self.num_batch, self.num*self.num_obs  )), -dist_obs), axis = 1)
    ...
    ###################################
    @partial(jit, static_argnums=(0,),backend="gpu")
    def compute_projection(self, x_obs_trajectory, y_obs_trajectory, d_obs, alpha_obs, alpha_a, d_a, alpha_v, d_v, lamda_x, lamda_y, b_eq_x, b_eq_y, sol_x_bar, sol_y_bar ):

        b_projection_x = sol_x_bar
        b_projection_y = sol_y_bar

        temp_x_obs = d_obs*jnp.cos(alpha_obs)*self.a_obs
        b_obs_x = x_obs_trajectory.reshape(self.num*(self.num_obs_proj))+temp_x_obs
            
        temp_y_obs = d_obs*jnp.sin(alpha_obs)*self.b_obs
        b_obs_y = y_obs_trajectory.reshape(self.num*(self.num_obs_proj))+temp_y_obs
    ...
    def initial_alpha_d(self, x_guess, y_guess, xdot_guess, ydot_guess, xddot_guess, yddot_guess, x_obs_trajectory, y_obs_trajectory, lamda_x, lamda_y):
        ...
        wc_alpha = wc_alpha.reshape(self.num_batch, self.num*(self.num_obs_proj))
        ws_alpha = ws_alpha.reshape(self.num_batch, self.num*(self.num_obs_proj))
        ...
        d_obs = c2_d/c1_d
        d_obs = jnp.maximum(jnp.ones((self.num_batch,  self.num*(self.num_obs_proj)   )), d_obs   )
        ####################### velocity terms
    ...
    @partial(jit, static_argnums=(0, ))	
    def compute_projection_sampling(self, key, sol_x_bar, sol_y_bar, x_obs_trajectory_proj, y_obs_trajectory_proj, lamda_x, lamda_y, x_guess, y_guess,  xdot_guess, ydot_guess, xddot_guess, yddot_guess, initial_state, x_fin, y_fin):

        b_eq_x, b_eq_y = self.compute_boundary_vec(initial_state, x_fin, y_fin)
        alpha_obs_init, d_obs_init, alpha_a_init, d_a_init, alpha_v_init, d_v_init, lamda_x_init, lamda_y_init = self.initial_alpha_d(x_guess, y_guess,  xdot_guess, ydot_guess, xddot_guess, yddot_guess, x_obs_trajectory_proj, y_obs_trajectory_proj , lamda_x, lamda_y)
        ...
        def lax_projection(carry, proj_iter ):
            ...
            c_x, c_y, x, y, xdot, ydot, xddot, yddot = self.compute_projection(x_obs_trajectory_proj, y_obs_trajectory_proj, d_obs, alpha_obs, alpha_a, d_a, alpha_v, d_v, lamda_x, lamda_y, b_eq_x, b_eq_y,sol_x_bar, sol_y_bar )
            alpha_obs, d_obs, alpha_a, d_a, alpha_v, d_v, lamda_x, lamda_y, res_norm_batch = self.compute_alph_d_proj( x, y, xdot, ydot, xddot, yddot, x_obs_trajectory_proj, y_obs_trajectory_proj, lamda_x, lamda_y)
    ...
    @partial(jit, static_argnums=(0,))
    def compute_cost_batch(self, x, y, xdot, ydot, xddot, yddot, x_project, y_project, res_norm_batch, x_fin, y_fin,  x_obs_trajectory, y_obs_trajectory):
        ...
        wc_alpha = wc_alpha.reshape(self.ellite_num_const, self.num*self.num_obs)
        ws_alpha = ws_alpha.reshape(self.ellite_num_const, self.num*self.num_obs)

        dist_obs = -wc_alpha**2/(self.a_obs**2)-ws_alpha**2/(self.b_obs**2)+1

        clearance_cost = - jnp.min(dist_obs, axis = 1)
        cost_obs = jnp.linalg.norm(jnp.maximum(jnp.zeros(( self.ellite_num_const, self.num*self.num_obs  )), dist_obs), axis = 1)

        cost_smoothness =  ( (jnp.linalg.norm(xddot, axis = 1)) + (jnp.linalg.norm(yddot, axis = 1)) )
        cost_track = jnp.linalg.norm(x-x_project, axis = 1)+jnp.linalg.norm(y-y_project, axis = 1) 
        cost_batch = 1.0* res_norm_batch + 0.25*cost_smoothness + 0.1* cost_track + 1.0*cost_obs + 1.0*clearance_cost
    ...
    @partial(jit, static_argnums=(0,))	
    def compute_shifted_samples(self, key, cost_batch, c_x_ellite, c_y_ellite, x_obs_trajectory, y_obs_trajectory,i, c_mean_prev, c_cov_prev):
    ...
    #####################################################
    @partial(jit, static_argnums=(0, ))
    def compute_controls(self, c_x_best, c_y_best, delta_t):
        ...
        ax_control = (xdot_best[self.num_sample] - xdot_best[0]) /  ((self.num_sample-1)*0.1) 
        ay_control = (ydot_best[self.num_sample] - ydot_best[0]) /  ((self.num_sample-1)*0.1)
        
        return vx_control, vy_control, ax_control, ay_control

    ##############################################
    @partial(jit, static_argnums=(0, ))	
    def compute_cem(self, key, initial_state, x_fin, y_fin, lamda_x, lamda_y, x_obs_trajectory, y_obs_trajectory, x_obs_trajectory_proj, y_obs_trajectory_proj, sol_x_bar, sol_y_bar, x_guess, y_guess,  xdot_guess, ydot_guess, xddot_guess, yddot_guess, x_waypoint,  y_waypoint, arc_vec, c_mean, c_cov):
        ...
        for i in range(0, self.maxiter_cem):
            ...
            c_x, c_y, x, xdot, xddot, y, ydot, yddot, res_norm_batch =  self.compute_projection_sampling(key, sol_x_bar, sol_y_bar, x_obs_trajectory_proj, y_obs_trajectory_proj, lamda_x, lamda_y, x_guess, y_guess,  xdot_guess, ydot_guess, xddot_guess, yddot_guess, initial_state, x_fin, y_fin)
            ...
            cost_batch  = self.compute_cost_batch(x_ellite, y_ellite, xdot_ellite, ydot_ellite, xddot_ellite, yddot_ellite, x_project_ellite, y_project_ellite, res_norm_batch_proj, x_fin, y_fin, x_obs_trajectory, y_obs_trajectory, )
                    
            key, subkey = random.split(key)
            sol_x_bar, sol_y_bar, x_guess, y_guess, xdot_guess, ydot_guess, xddot_guess, yddot_guess, c_mean_prev, c_cov_prev = self.compute_shifted_samples(key, cost_batch, c_x_ellite, c_y_ellite,  x_obs_trajectory, y_obs_trajectory, i, c_mean_prev, c_cov_prev)
        ...
        xdot_best = xdot_ellite[idx_min]
        ydot_best = ydot_ellite[idx_min]

        x_guess_per  = x_guess[idx_sort[0:self.num_sample_warm]]
        y_guess_per  = y_guess[idx_sort[0:self.num_sample_warm]]


        return  x_ellite, y_ellite, x, y, c_x_best, c_y_best, x_best, y_best, xdot_best, ydot_best, x_guess_per, y_guess_per